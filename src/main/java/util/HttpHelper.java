...
public class HttpHelper {
		...
		protected static final String DEFAULT_CHARSET = "UTF-8";// 默认请求编码
		protected static final int DEFAULT_SOCKET_TIMEOUT = 6000 * 30;// 默认等待响应时间(毫秒)
		protected static final int DEFAULT_RETRY_TIMES = 0;// 默认执行重试的次数
	    ...
	    public static CloseableHttpClient createHttpClient(int retryTimes, int socketTimeout) {
	        ...
	        System.out.println("httpClientBuilder:"+ httpClientBuilder);
	        if (retryTimes > 0) {
	     ...
	     * token
	     */
	    public static HttpResult executeTokenGet(String url, String charset, int socketTimeout,String token) throws IOException {
	        CloseableHttpClient httpClient = createHttpClient(socketTimeout);
	        return executeTokenGet(httpClient, url, null, null, charset, true,token);
	    }

	    public static HttpResult executeTokenGet(CloseableHttpClient httpClient, String url, String referer, String cookie, String charset, boolean closeHttpClient, String token) throws IOException {
	        CloseableHttpResponse httpResponse = null;
	        try {
	            charset = getCharset(charset);
	            httpResponse = executeTokenGetResponse(httpClient, url, referer, cookie,token);
	            //Http请求状态码
	    ...
	    public static CloseableHttpResponse executeTokenGetResponse(CloseableHttpClient httpClient, String url, String referer, String cookie, String token) throws IOException {
	        if (httpClient == null) {
	    ...
	    public static CloseableHttpResponse executeGetResponse(CloseableHttpClient httpClient, String url, String referer, String cookie) throws IOException {
			...
			String tid = StrUtil.toString(Thread.currentThread().getId());
			if (StrUtil.isNotBlank(tid)) {
				get.setHeader("tid", tid);
			}
	        return httpClient.execute(get);
	    }
		...
		public static HttpResult executeJpushPost(String url, Object paramsObj, String charset, int socketTimeout, String authorization) throws IOException {
			// TODO Auto-generated method stub
			CloseableHttpClient httpClient = createHttpClient(socketTimeout);
	        return executeJpushPost(httpClient, url, paramsObj, null, null, charset, true,authorization);
		}

		private static HttpResult executeJpushPost(CloseableHttpClient httpClient, String url, Object paramsObj,
                                                   Object object, Object object2, String charset, boolean closeHttpClient, String authorization) throws IOException {
			// TODO Auto-generated method stub
			 CloseableHttpResponse httpResponse = null;
		        try {
		            charset = getCharset(charset);
		            httpResponse = executePostJpushResponse(httpClient, url, paramsObj, null, null, charset,authorization);
		            //Http请求状态码
		            Integer statusCode = httpResponse.getStatusLine().getStatusCode();
		            String content = getResult(httpResponse, charset);
		            return new HttpResult(statusCode, content);
		...
		private static CloseableHttpResponse executePostJpushResponse(CloseableHttpClient httpClient, String url,
                                                                      Object paramsObj, Object referer, Object cookie, String charset, String authorization) throws ClientProtocolException, IOException {
			// TODO Auto-generated method stub
			 if (httpClient == null) {
		            httpClient = createHttpClient();
		        }
	 ...
	 * 拼接get请求url
	 */
	public static String getAppendHttpGetUrl(String url, Map<String, String> reqUrlMap) throws IOException {
		if (reqUrlMap != null) {
			Iterator<String> it = reqUrlMap.keySet().iterator();
			StringBuffer sb = null;
			while (it.hasNext()) {
				String key = it.next();
				String value = reqUrlMap.get(key);
				if (sb == null) {
					sb = new StringBuffer();
					sb.append("?");
				} else {
